    ...
    (set, get) => {
      ...
      return {
        ...
        patchJob: (jobId, patch) => {
          commitJobs(get().jobs.map((j) => (j.id === jobId ? touch(j, patch) : j)));
        },
        ...
        submitReading: (jobId, pack, raw, optionId) => {
          ...
          if (unusualGate) {
            ...
            commitJobs(get().jobs.map((j) => (j.id === jobId ? next : j)));
          ...
          commitJobs(get().jobs.map((j) => (j.id === jobId ? updated : j)));
          return {
        ...
        resetPending: (jobId) => {
          commitJobs(
            get().jobs.map((j) => (j.id === jobId ? touch(j, { pending: undefined }) : j)),
          );
        },
        deleteJob: (jobId) => {
          commitJobs(get().jobs.filter((j) => j.id !== jobId));
        },
        ...
        setIncludeAiInReport: (jobId, include) => {
          commitJobs(
            get().jobs.map((j) => (j.id === jobId ? touch(j, { includeAiInReport: include }) : j)),
          );
        },
        confirmReport: (jobId) => {
          commitJobs(
        ...
        jumpToStep: (jobId, stepId, reason) => {
          ...
          commitJobs(
            get().jobs.map((j) => (j.id === jobId ? applyJumpToStep(j, stepId, reason, at) : j)),
          );
        },
      };
    },
