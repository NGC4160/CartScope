...
export function buildAcDrive(spec: AcDriveSpec): ModelPack {
  ...
  const { components, wires, testPoints } = acPowerLayout({
    ...
    tow: {
      name: "Run / Tow switch",
      description: "Run/Tow is the work/drive switch on the battery well. RUN = ready to drive. TOW = off for work. Set TOW before jumpers or trailer loading. Towing an AC cart in RUN can wreck the speed box (controller).",
    ...
    solenoid: {
      ...
      description:
        "The big click switch (solenoid) is the main power switch. It sends pack power so the speed box (controller) can fill up. A click is required. The cart still will not move if the electric park brake is not released. RXV 12576 DC Bus Timeout: pack ≥ 42 V, then check power across the metal pads inside.",
      ...
      expectedValues: [
        ...
        { label: "Drop across metal pads (12576/12833)", value: "If 3–24 V, suspect the big click switch" },
      ],
    },
    controller: {
    ...
    throttle: {
      ...
      description: `The gas pedal sensor (ITS) tells the cart how hard you press the pedal. Speed-box 5 V reference in. Signal ${spec.throttleUp.label} at rest, ${spec.throttleFull.label} full. RXV: the number must be ≤ 0.85 V when the pedal switch closes.`,
    ...
    brake: {
      name: spec.brakeName,
      description: `Spring-applied, electrically released rear park brake. If the speed box does not turn on the brake coil, the cart will not move even with a clicking big click switch. Factory coil ${spec.brakeOhms.label}.`,
      commonFailures: ["Open brake coil", "Speed box not telling it to let go", "Stuck mechanically", "Brake sensor error"],
      expectedValues: [
  ...
  const steps: Record<string, DiagnosticStep> = {
    "rno-setup": obs(
      ...
      "Now do this. Put Run/Tow in RUN (ready to drive). Turn the key ON. Put FNR (the direction switch) in Forward, not Neutral. Unplug the charger. Write down any speed-box error / warning on the display or LED before you clear it.",
    ...
    "rno-click": obs(
      "rno-click",
      "Listen for the big click switch",
      "The big click switch (solenoid) is the main power switch. Now do this. Key ON, direction switch (FNR) Forward, light pedal. Listen for the click. A click without movement points at the park brake, not the click switch.",
      ...
      "Click",
      [
        { id: "click", label: "The big click switch clicked", result: "pass" },
    ...
    "rno-logic": volt(
      "rno-logic",
      "Speed-box logic power",
      "RUN, key ON. Read logic feed at the speed-box signal plug (Run/Tow output).",
      `Factory book: ${M} — Run/Tow and logic power`,
      ["s1", "a1", "w-s1-a"],
      "Logic feed at the speed box, RUN + key ON",
    ...
    "rno-coil": volt(
      "rno-coil",
      "Click-switch coil power",
      "The coil is the small magnet wires that pull the click switch in. Now check if the power is flowing. Voltage is how strong the electric power is. Pedal down, direction switch Forward. Measure across the coil.",
    ...
    "rno-brake": volt(
      ...
      "The park brake is the electric park brake. The big click switch clicked but the cart did not move. Now check if the power is flowing. Voltage is how strong the electric power is. Read power at the park-brake coil while in gear with a light pedal. No power = speed box not telling it to let go. Power present = stuck brake or motor speed sensor. Then read coil ohms.",
    ...
    "rno-phase": ohm(
      ...
      `TOW, main B− off. Unplug U, V, W from the speed box. Read U–V, V–W, W–U. Factory ${spec.phaseOhms.label} and all three nearly equal. Out of range = replace motor (RXV 8976 / 9024 ACTION 1). In range and still dead = speed box (ACTION 2).`,
      `Factory book: ${M} — Motor tests / errors 8976 and 9024`,
      ["m1", "w-u", "w-v", "w-w"],
      "U–V phase ohms",
      "Ohms between phase U and V at the motor, cables off the speed box.",
      ...
      { caution: "Take off the main negative. Never probe U/V/W as pack voltage while the speed box is running." },
    ),
    "rno-enc": obs(
      "rno-enc",
    ...
    "rlimp-reset": obs(
      "rlimp-reset",
      "Limp-mode trigger",
      "Reduced-speed / limp is a stored speed-box fault. Do not reset first — name the trigger (brake switch, encoder / motor speed sensor, throttle 5 V, overtemp). After the cause is fixed: key-cycle, then if needed disconnect main B− for 30 seconds.",
    ...
    "rint": obs(
      ...
      "The encoder is the motor speed sensor. If AC cuts in and out: wiggle the speed-box signal plug, encoder cap, and brake connector with a light pedal, wheels blocked.",
  ...
  const diagnoses: Record<string, Diagnosis> = {
    ...
    "rdx-tow": dx("rdx-tow", "Run/Tow or logic power is open", "The speed box is not powered.", "Run/Tow switch or logic fuse.", "Replace Run/Tow if it does not pass power in RUN.", [{ name: "Run/Tow switch" }, { name: "Control fuse" }], "replace"),
    "rdx-throttle": dx("rdx-throttle", "Gas pedal sensor is not in range", "The gas pedal sensor (ITS) never reaches the window, 5 V is missing, or the number is > 0.85 V when the pedal switch closes.", "Failed pedal sensor or speed-box 5 V supply.", "Check 5 V at the pedal. If 5 V is good, replace the gas pedal sensor. If 5 V is missing, look at the speed-box supply (20755).", [{ name: "Gas pedal sensor (ITS)" }], "replace"),
    "rdx-fnr": dx("rdx-fnr", "Direction switch (FNR)", "The speed box is not seeing a direction request (25104 Direction Error), so it will not turn on the big click switch or let the brake go.", "Direction switch or harness.", "Replace the direction switch (FNR). Make sure Neutral really opens.", [{ name: "Direction switch (FNR)" }], "replace"),
    "rdx-solenoid": dx("rdx-solenoid", "Big click switch failed", "Coil power is there but the unit does not click, or the metal pads drop 3–24 V (12576 / 12818 ACTION 3).", "Open/short coil, stuck plunger, or burned metal pads inside.", "Replace the big click switch (solenoid). Then re-test brake release.", [{ name: "48 V big click switch" }], "replace"),
    "rdx-brake-cmd": dx("rdx-brake-cmd", "Park brake not told to let go", "The big click switch clicked; brake-release power is missing. The cart cannot move with the spring brake on.", "Speed-box brake output, stored brake fault, or open BR1 feed.", "Read BR1 coil ohms next. If the coil is good, fix the speed-box brake output. Do not defeat the brake by hand to make it roll.", [{ name: "Electric park-brake assembly" }], "replace"),
    ...
    "rdx-controller": dx("rdx-controller", "AC speed-box fault", "Pack, big click switch, brake release, motor phases, and speed-sensor supply all passed — or 8976/9024 ACTION 2 after a good motor.", "Failed speed box (often after being towed in RUN).", "Replace the speed box. After install, clear limp with a key-cycle, then a short roll test.", [{ name: spec.controllerName }], "replace"),
    ...
    "rdx-intermittent": dx("rdx-intermittent", "Cuts in and out — speed box inside", "Wiggling the outside plugs did not make it drop.", "Heat limp or a cracked solder joint in the speed box.", "Heat-cycle road test, then replace the speed box if it drops out.", [{ name: spec.controllerName }], "service"),
    ...
    "rdx-hs-temp": dx("rdx-hs-temp", "17168 Heat Sink Temp High", "Speed box ≥ 120 °C. Outside heatsink should be < 80 °C (176 °F).", "Overload or blocked heatsink.", "Let it cool. Reduce payload/grade. If it overheats with no load, replace the speed box.", [{ name: spec.controllerName }], "service"),
    "rdx-5v": dx("rdx-5v", "20755 5 V supply low or high", "Often a short in the 5 V wire harness, or speed-box inside 5 V. 20753 15 V supply low is often a shorted reverse alarm (100–500 Ω) or relay (1–50 Ω).", "Shorted 5 V harness, reverse alarm, or speed box.", "Unplug the 23-pin. If 5 V comes back, repair the harness/gas pedal sensor. Reverse alarm 100–500 Ω; if not, replace the alarm. Otherwise replace the speed box.", [{ name: "Gas pedal sensor / 5 V harness" }, { name: "Reverse alarm" }, { name: spec.controllerName }], "replace"),
  };

  const symptoms: SymptomDef[] = [
    { id: "no-operation", label: "Cart will not run", summary: "No roll. The big click switch may or may not click. Pack ≥ min V, then brake, then U/V/W.", manualSection: `${M} — Vehicle will not operate`, startStepId: "rno-setup" },
    ...
    { id: "park-brake", label: "Park brake will not let go", summary: "The big click switch clicks; cart is locked. Coil " + spec.brakeOhms.label + ".", manualSection: `${M} — Parking brake will not release`, startStepId: "rno-brake" },
    { id: "limp", label: "Reduced speed / limp mode", summary: "Moves slowly with a stored speed-box fault.", manualSection: `${M} — Reduced speed`, startStepId: "rlimp-reset" },
