...
export function buildEzgoDc(spec: EzgoDcSpec): ModelPack {
  ...
  const { components, wires, testPoints } = dcPowerLayout({
    battery: {
      name: "Battery pack",
      description: `${spec.packCells}. The pack is the whole set of batteries. Check the pack first if the cart has no power. One weak battery can take power from the controller and the motor.`,
    ...
    tow: {
      ...
      description:
        spec.towDesc ??
        "This is the Run/Tow switch (Tow = off for work. Run = ready to drive.). Run sends power to the controller on the 16-pin red wire (KSI). Tow cuts that power. After you hook the batteries back up, leave it in Tow for 30 seconds so the controller can fill up. Skip this and a TCT cart often will not go.",
    ...
    solenoid: {
      name: "Main solenoid",
      description:
        "The solenoid sends power to the motor. It has four posts. The two big posts feed B+ to the controller. The TCT 1206HB can show MAIN WELDED, MAIN COIL OPEN, MAIN DROPOUT 1/2, or MAIN DRIVER ON/OFF for this part. A click does not prove the metal pads inside (contacts) are passing power.",
    ...
    motor: {
      ...
      description:
        spec.family === "tct"
          ? "TCT drive motor with its own field (magnet wires in the motor). FIELD MISSING on the 1206HB means the field or a field wire is broken. M− SHORTED means the controller did not see the armature (spinning part of the motor) running. SPEED SENSOR FAULT / MOTOR STALL means the tail speed sensor is bad or the motor is stuck."
    ...
    key: {
      name: "Key switch (KSI)",
      description: "The dash key is KSI (key switch input). It turns on the controller. HPD sets if you press the gas pedal before the key and the direction switch.",
    ...
    fr: {
      ...
      description: "F&R is the direction switch. It also tells the controller the cart is in gear. If it only fails one way, DCS often has burned field metal pads (contacts). TCT uses controller field current plus this switch.",
    ...
    fuse: {
      name: "Control fuse",
      description: "Small in-line fuse that feeds the key and the coil (the small magnet wires that pull the solenoid in).",
  ...
  const steps: Record<string, DiagnosticStep> = {
    ...
    "tno-pack": volt(
      "tno-pack",
      "Read pack voltage",
      `Now check if the power is flowing. Voltage is how strong the electric power is. Turn the key OFF. Put your meter on the main pack posts. ${spec.packCells}. Do not test a controller on a dead pack.`,
    ...
    "tno-click": obs(
      ...
      "The solenoid sends power to the motor. Now do this. Key ON. F&R direction switch in Forward. Run/Tow in Run. Press the pedal. Listen at the solenoid. TCT ITS (gas pedal sensor) should be " +
        spec.itsClick.label +
        " when the solenoid first closes.",
    ...
    "tno-16red": volt(
      "tno-16red",
      "16-pin red / KSI — controller power",
      "Now check if the power is flowing. Voltage is how strong the electric power is. Look at this number. The red wire at the controller plug must show pack voltage in Run. If it is dark, the Run/Tow switch or the red lead is broken. After you hook the batteries back up, wait 30 seconds in Tow on TCT before this reading counts.",
      `Factory book: ${M} — Controller 16-pin, red / KSI`,
      ["a1", "s1", "w-s1-a1"],
      "16-pin red (KSI) to B−, Run, key ON",
      "DC volts. Poke the probe on the red wire at the controller plug.",
      "Pack voltage",
    ...
    "tno-its": volt(
      ...
      spec.family === "tct"
        ? "The ITS is the gas pedal sensor. Now check if the power is flowing. Voltage is how strong the electric power is. Put the + probe on the ITS. Put − on B−. Look at this number. You should see 1.0 V ± 0.3 V when the solenoid clicks. You should see 2.7 V ± 0.5 V with the pedal all the way down. A bad number is THROTTLE FAULT. That can mean no click or a slow cart."
        : spec.throttleDesc,
      `Factory book: ${M} — ${spec.throttleName}`,
      ["s4", "w-mcor-sig", "a1"],
      spec.family === "tct" ? "ITS voltage when the solenoid clicks (or first move)" : "Gas pedal sensor signal, first move",
    ...
    "tno-its-full": volt(
      ...
      spec.family === "tct"
        ...
        : "Now check if the power is flowing. Voltage is how strong the electric power is. Press the pedal all the way down. The gas pedal sensor number must sit in the controller window. If it does not, the cart will limit power.",
    ...
    "tno-coil": volt(
      ...
      "Now check if the power is flowing. Voltage is how strong the electric power is. The coil is the small magnet wires that pull the solenoid in. Key ON. F&R direction switch in Forward. Pedal down. Read voltage across the small coil posts. Look at this number. A dark coil with a good ITS (gas pedal sensor) points to the key, the direction switch, or the fuse.",
    ...
    "tno-contacts": volt(
      "tno-contacts",
      "Solenoid L2 voltage",
      "Now check if the power is flowing. Voltage is how strong the electric power is. It clicked. Hold the pedal. Look at this number. L2 (controller B+ side) to pack B− should be pack voltage. Also check the big posts with the coil off. If that path is connected all the way, that is MAIN WELDED. Replace the solenoid.",
      `Factory book: ${M} — Solenoid contacts / MAIN WELDED`,
      ["k1", "a1", "w-k-a"],
      "L2 to pack B−, pedal held",
      "DC volts on solenoid L2.",
    ...
    "tno-field": ohm(
      "tno-field",
      "Motor field ohms",
      "Ohms (Ω) tell you how hard it is for power to flow. OL means the path is broken. The field is the magnet wires in the motor. A broken field is 1206HB FIELD MISSING (solenoid closed, response 2+8).",
    ...
    "tsl-its": volt(
      ...
      "The ITS is the gas pedal sensor. Now check if the power is flowing. Voltage is how strong the electric power is. Slow TCT/PDS carts often have a gas pedal sensor that never reaches full voltage. That is scaled throttle (controller response 8). Write down the click number and the full-pedal number.",
    ...
    "tdir-fr": cont(
      ...
      "F&R is the direction switch. The solenoid clicks. But only one way works. On DCS/PDS, check the field metal pads (contacts) in the F&R first. On TCT, also think FIELD MISSING on one field lead. This check asks: is that path connected all the way?",
    ...
    "tint-wiggle": obs(
      "tint-wiggle",
      "Wiggle test — 16-pin and solenoid",
      "The solenoid sends power to the motor. If it cuts in and out, do this. Block the wheels. Use a light pedal. Wiggle the 16-pin. Wiggle the ITS (gas pedal sensor) wires. Wiggle the solenoid cable ends. MAIN DROPOUT 1/2 is a solenoid that opens while you drive, or while the cart is slowing itself with the motor.",
    ...
    "tfault": obs(
      "tfault",
      "1206HB / controller fault code",
      spec.family === "tct"
        ? "The controller is a Curtis 1206HB-5201. Factory book TXT TCT Section E lists these codes:\nHW FAILSAFE — controller or power cables hooked up wrong. Cycle the key (KSI) to try again.\nFIELD MISSING — the field (magnet wires in the motor) or a field wire is broken.\nM− SHORTED — controller or power cables are shorted.\nCURRENT SENSE FAULT — controller, or too much power at the plug.\nMAIN DROPOUT 1/2 — the solenoid opened while driving, or while the cart was slowing itself with the motor.\nMAIN DRIVER OFF/ON — controller FET, or pin 12 shorted to ground.\nMOTOR STALL — motor is stuck, or the motor speed sensor is bad.\nMAIN COIL OPEN — the coil (small magnet wires that pull the solenoid in) or its wiring is broken.\nSPEED SENSOR FAULT — motor speed sensor or its wiring.\nMAIN WELDED — solenoid stuck shut (metal pads welded).\nHPD — key, direction switch, and gas pedal were used in the wrong order. It clears when the pedal is under 25 %.\nTHERMAL CUTBACK — hotter than 85 °C or colder than −25 °C, heavy load, or how the controller is mounted.\nOVERVOLTAGE / LOW BATTERY VOLTAGE — the battery pack.\nTHROTTLE FAULT — gas pedal sensor (ITS) or its wiring.\nWhat the controller does: 1 armature 0 · 2 field 0 · 3 solenoid off · 4 limp 75 % · 5 slow armature limit · 8 throttle scaled to 0 · 9 walk-away."
        : "Read the controller status LED / handset. PDS and DCS store gas-pedal, HPD, thermal, and field faults. Name the code before you replace the controller.",
  ...
  const diagnoses: Record<string, Diagnosis> = {
    ...
    "tdx-pack": dx("tdx-pack", "Battery pack is too low", "The pack cannot feed the controller or the motor (LOW BATTERY VOLTAGE on 1206HB).", "Dead or failed batteries, or crusty terminals.", "Charge, then load-test. Replace as a matched set if more than one battery is weak.", [{ name: spec.packCells }], "replace"),
    "tdx-tow": dx("tdx-tow", "Run/Tow or 16-pin red is broken", "Controller KSI (turn-on power) is missing.", "Run/Tow switch or the red lead to the 16-pin.", "Replace the Run/Tow switch if it does not pass pack voltage in Run. Fix the red lead.", [{ name: "Run/Tow switch" }], "replace"),
    ...
    "tdx-key-fr": dx("tdx-key-fr", "Key or F&R path is broken", "ITS (gas pedal sensor) is in range. But the solenoid coil is not seeing pack voltage.", "Key switch, F&R in-gear switch, or a blown fuse.", "Check F1. Then check key output. Then check the F&R in-gear small switch.", [{ name: "Key switch" }, { name: "F&R switch" }], "replace"),
    "tdx-solenoid-coil": dx("tdx-solenoid-coil", "Solenoid coil is broken or shorted", `Coil ohms are out of ${spec.solenoidCoil.label}. MAIN COIL OPEN on 1206HB.`, "Failed coil (small magnet wires).", "Replace the 4-post solenoid.", [{ name: `${spec.voltage} V 4-terminal solenoid` }], "replace"),
    "tdx-solenoid-mech": dx("tdx-solenoid-mech", "Solenoid is stuck", "Coil ohms and coil voltage are good. The plunger (moving pin) did not move.", "Stuck plunger (moving pin).", "Replace the solenoid.", [{ name: `${spec.voltage} V 4-terminal solenoid` }], "replace"),
    "tdx-solenoid-contacts": dx("tdx-solenoid-contacts", "Solenoid metal pads are not passing power", "Click without pack voltage on L2, or the big posts are welded (MAIN WELDED).", "Rough or welded big contacts (metal pads).", "Replace the solenoid. Look at the motor and controller for the overload.", [{ name: `${spec.voltage} V 4-terminal solenoid` }], "replace"),
    ...
    "tdx-controller": dx("tdx-controller", `${spec.controllerName} fault`, "Logic power, gas pedal sensor, solenoid metal pads, and motor ohms passed — or HW FAILSAFE / CURRENT SENSE / MAIN DRIVER is stored.", "Failed power section or power cables hooked up wrong (HW FAILSAFE).", "Check power-cable direction. Push the 16-pin in again. Then replace the controller.", [{ name: spec.controllerName }], "replace"),
    "tdx-slow": dx("tdx-slow", "Low power — drag or controller limit", "Gas pedal sensor and pack under load passed. What’s left is brake drag, soft tires, or a controller power / heat limit.", "Mechanical drag or heat limit.", "Check brakes and tires first. Then check the controller.", [], "service"),
    ...
    "tdx-harness": dx("tdx-harness", "Loose wires or cable end", "A wiggle made it drop out (MAIN DROPOUT if it is the solenoid).", "16-pin, ITS (gas pedal sensor) plug, or solenoid cable end.", "Fix the connection that dropped out.", [], "service"),
    "tdx-intermittent": dx("tdx-intermittent", "Cuts in and out — controller inside", "Wiggling the outside plugs did not make it drop.", "Heat inside, or a cracked solder joint.", "Heat-cycle road test. Then replace the controller if it drops out.", [{ name: spec.controllerName }], "service"),
    "tdx-hpd": dx("tdx-hpd", "HPD — high pedal disable", "The controller powered up with key, direction switch, and gas pedal already pressed. Response 8 (throttle scaled to 0) until the pedal is under 25 %.", "Wrong key/pedal order, or a gas pedal sensor stuck off idle.", "Foot off the pedal. Key OFF. Then key ON. Then F&R direction switch. Then pedal. If HPD comes back with the pedal up, replace the ITS (gas pedal sensor).", [{ name: spec.throttleName, notes: "If HPD comes back with the pedal up" }], "info"),
    "tdx-thermal": dx("tdx-thermal", "Thermal cutback (too hot or too cold)", "Temperature > 85 °C or < −25 °C. Response 5 (slow armature current limit).", "Overload, dragging brakes, or the controller is not on a clean heat sink.", "Let it cool. Check the mount and the brakes. If it cuts back when cold, replace the controller.", [{ name: spec.controllerName, notes: "If it cuts back when cold" }], "service"),
    "tdx-speed": dx("tdx-speed", "Speed sensor / motor stall", "No speed pulses with the solenoid closed (SPEED SENSOR FAULT), or high armature current with no pulses (MOTOR STALL).", "Failed tail sensor, broken wiring, or the motor is really stuck.", "Look at the speed-sensor plug. Replace the sensor if it does not pulse while you turn the shaft in Tow. Do not hold a cart on a hill with the pedal.", [{ name: "Motor speed sensor" }], "replace"),
  };

  const symptoms: SymptomDef[] = [
    { id: "no-operation", label: "Will not run", summary: "No roll, with or without a click. Pack first. Then KSI. Then ITS (gas pedal sensor). Then the solenoid. Then the motor.", manualSection: `${M} — Will not operate`, startStepId: "tno-setup" },
    ...
    { id: "fault-code", label: "Controller fault code", summary: spec.family === "tct" ? "1206HB-5201 code: HW FAILSAFE, FIELD MISSING, THROTTLE FAULT, HPD, THERMAL CUTBACK, …" : "Stored controller fault.", manualSection: `${M} — Controller faults`, startStepId: "tfault" },
    { id: "one-direction", label: "Runs one way only", summary: "The solenoid clicks. One direction is dead.", manualSection: `${M} — One direction only`, startStepId: "tdir-fr" },
